...
class BaseModel():
    ...
    def load_networks(self, which_epoch):
        ...
        print(f'------- loading network from {which_epoch} iter. -----------')
        for name in self.model_names:
            if isinstance(name, str):
    ...
    def save_results(self, save_data, score=None, data_name='none', masks=None):
        """Save the training or testing results to disk"""
        img_names = self.get_image_names()

        for i in range(save_data.size(0)):
            print('process image ...... %s' % img_names[i])
            # short_path = ntpath.basename(img_names[i])  # get image path
            name = os.path.splitext(img_names[i])[0]
            ...
            img_numpy = util.tensor2im(save_data[i].data)  # RGB
