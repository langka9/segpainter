...
class Visualizer():
    def __init__(self, opt):
        ...
        self.name = os.path.basename(opt.run_dir)

        if self.display_id > 0:
            import visdom
            self.vis = visdom.Visdom(env=self.name, port = opt.display_port)
        ...
        self.log_name = os.path.join(opt.run_dir, 'loss_log.txt')
    ...
    def plot_current_errors(self, iters, errors):
        ...
        for k, v in errors.items():
            if self.plot_data.get(k, None) is None:
                self.plot_data[k] = {'X': [], 'Y': [], 'legend': []}
            self.plot_data[k]['X'].append(iters)
            self.plot_data[k]['Y'].append(v)
            self.plot_data[k]['legend'].append(k)
            self.vis.line(
                X=np.array(self.plot_data[k]['X']),
                Y=np.array(self.plot_data[k]['Y']),
                opts={'title': self.name + ' loss over time',
                    'legend': self.plot_data[k]['legend'],
                    'xlabel': 'iterations',
                    'ylabel': 'loss'},
                win=self.display_id + idx)
            idx += 1
