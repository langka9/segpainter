...
class TextTransforms(TransformsConfig):

	def __init__(self, opts):
		super(TextTransforms, self).__init__(opts)
...
class SoftSegToImageTransforms(TransformsConfig):
	...
	def get_transforms(self):
		transforms_dict = {
			...
			'transform_source': transforms.Compose([
				transforms.Resize((256, 256)),
				augmentations.ToSoftOneHot(self.opts.label_nc),
				transforms.ToTensor()]),
			'transform_test': transforms.Compose([
				transforms.Resize((256, 256)),
				transforms.ToTensor(),
